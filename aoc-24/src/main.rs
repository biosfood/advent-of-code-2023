...
fn main() {
    ...
    for i in 0..particles.len() {
        for j in (i+1)..particles.len() {
            ...
            // println!("Particle {} and {} collide at {} and {}", i, j, t1, t2);
            // let position = (0..2).map(|index| particles[i].position[index] as i128 + t1 * particles[i].velocity[index] as i128).collect::<Vec<i128>>();
            ...
            intersections += 1;
            // let position1 = (0..2).map(|index| (particles[i].position[index]) + t1 * (particles[i].velocity[index]));
            // let position2 = (0..2).map(|index| (particles[j].position[index]) + t2 * (particles[j].velocity[index]));
            // println!("Particle {} and {} collide at {:?} and {:?}", i, j, position1.collect::<Vec<i128>>(), position2.collect::<Vec<i128>>());
        }
    }
    ...
    for i in 0..particles.len() {
        ...
        if particle.collision_partner == usize::MAX || particle.collision_time < f128::from(0) {
            // no collision (weird)
            continue;
        }
        if particles[particle.collision_partner].collision_partner != i {
            // println!("particle {} and {} are not colliding, because another collision occured earlier", i, particle.collision_partner);
            continue;
        }
        let position = (0..2).map(|index| f128::from(particles[i].position[index]) + particle.collision_time * f128::from(particles[i].velocity[index])).collect::<Vec<f128>>();
        let mut in_bounds = true;
        for pos in &position {
            if *pos < f128::from(200000000000000 as i128) || *pos > f128::from(400000000000000 as i128) {
                in_bounds = false;
        ...
        println!("Particle {} and {} collide at {:?}", i, particle.collision_partner, position);
        collisions += 1;
    }
    println!("{} collisions (really {})", collisions, collisions / 2);
}
